...
import { KeyboardAvoidingView, ScrollView, TouchableOpacity, View, TouchableWithoutFeedback, Keyboard, Platform } from 'react-native';
...
export function MobileRechargeDeposit({ onBack, onSuccess }: MobileRechargeDepositProps) {
  ...
  const handleRecharge = async () => {
    ...
    const currentDeposit = depositInfo?.currentDeposit || 0;
    const totalAfterRecharge = currentDeposit + amount;
    const maxDeposit = 20000;

    if (totalAfterRecharge > maxDeposit) {
      const maxAllowed = maxDeposit - currentDeposit;
      haptics.error();
      toast.error(t('wallet.depositExceeded', { max: maxDeposit, allowed: maxAllowed }));
      return;
    }
            ...
            <View style={styles.gap12}>
              <Button 
                ...
                style={{ 
                  ...
                  opacity: (isProcessing || (!selectedAmount && !customAmount) || walletBalance < (selectedAmount || parseInt(customAmount) || 0) || ((depositInfo?.currentDeposit || 0) + (selectedAmount || parseInt(customAmount) || 0)) > 20000) ? 0.6 : 1
                }}
              >
                <View style={[styles.row, styles.gap8, styles.alignCenter]}>
                  <Shield size={16} color="white" />
                  <Text style={styles.ml8} color="white">
                    {isProcessing ? t('wallet.recharging') : t('wallet.rechargeAmount', { amount: (selectedAmount || customAmount || 0).toLocaleString() })}
                  </Text>
                </View>
              </Button>
