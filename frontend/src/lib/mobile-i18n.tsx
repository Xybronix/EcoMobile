...
const translations: Translations = {
  ...
  // Map
  'map.title': { fr: 'Vélos disponibles', en: 'Available Bikes' },
  'map.nearMe': { fr: 'Près de moi', en: 'Near Me' },
  'map.filters': { fr: 'Filtres', en: 'Filters' },
  'map.available': { fr: 'Disponible', en: 'Available' },
  'map.distance': { fr: 'Distance', en: 'Distance' },
  'map.battery': { fr: 'Batterie', en: 'Battery' },
...
export function MobileI18nProvider({ children }: { children: ReactNode }) {
  const [language, setLanguage] = useState<Language>(() => {
    const saved = localStorage.getItem('freebike_mobile_language');
    return (saved as Language) || 'fr';
  });

  const handleSetLanguage = (lang: Language) => {
    setLanguage(lang);
    localStorage.setItem('freebike_mobile_language', lang);
  };
  ...
  return (
    <MobileI18nContext.Provider value={{ language, setLanguage: handleSetLanguage, t }}>
      {children}
